...
class GitHubIdeasAgent:
    """An agent that creates GitHub issues from ideas."""
    ...
    def _build_agent(self) -> LlmAgent:
        """Builds the LLM agent for the GitHub ideas agent."""
        LITELLM_MODEL = os.getenv("LITELLM_MODEL", "gemini/gemini-2.5-flash")

        instruction = get_ui_prompt(self.base_url, GITHUB_IDEAS_UI_EXAMPLES)

        return LlmAgent(
            model=LiteLlm(model=LITELLM_MODEL),
            name="github_ideas_agent",
            description="An agent that creates GitHub issues from ideas.",
    ...
    async def stream(self, query, session_id) -> AsyncIterable[dict[str, Any]]:
        ...
        if self.a2ui_schema_object is None:
            logger.error(
                "--- GitHubIdeasAgent.stream: A2UI_SCHEMA is not loaded. "
        ...
        while attempt <= max_retries:
            attempt += 1
            logger.info(
                f"--- GitHubIdeasAgent.stream: Attempt {attempt}/{max_retries + 1} "
            ...
            if final_response_content is None:
                logger.warning(
                    f"--- GitHubIdeasAgent.stream: Received no final response content from runner "
                    f"(Attempt {attempt}). ---"
                )
            ...
            logger.info(
                f"--- GitHubIdeasAgent.stream: Validating UI response (Attempt {attempt})... "
            )
            try:
                ...
                if not json_string.strip() or json_string_cleaned == "[]":
                    logger.info(
                        "--- GitHubIdeasAgent.stream: Empty JSON list found. Assuming valid. ---"
                    )
                    is_valid = True
                    ...
                    logger.info(
                        "--- GitHubIdeasAgent.stream: Validating against A2UI_SCHEMA... "
                    )
                    jsonschema.validate(
                    ...
                    logger.info(
                        f"--- GitHubIdeasAgent.stream: UI JSON successfully parsed AND validated against schema. "
                ...
                logger.warning(
                    f"--- GitHubIdeasAgent.stream: A2UI validation failed: {e} (Attempt {attempt}) ---"
                )
                logger.warning(
            ...
            if is_valid:
                logger.info(
                    f"--- GitHubIdeasAgent.stream: Response is valid. Sending final response (Attempt {attempt}). "
                )
            ...
            if attempt <= max_retries:
                logger.warning(
                    f"--- GitHubIdeasAgent.stream: Retrying... ({attempt}/{max_retries + 1}) "
                )
                current_query_text = (
        ...
        logger.error(
            "--- GitHubIdeasAgent.stream: Max retries exhausted. Sending text-only error. ---"
        )
        yield {
            "is_task_complete": True,
            "content": (
